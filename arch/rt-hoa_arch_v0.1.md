...
> **Version tag:** `rt-hoa_arch_v0.1` — strict augmentation of `HOA_Chat_Full_Technical_Checkpoint_Augmented.md`.
...
### 13.2 Avoiding Blind Polling
- Daily cron satisfies MVP reliability, but Drive push notifications provide a smarter trigger. Register a `changes.watch` webhook per folder so Google notifies `/ingest/drive-webhook` whenever files change. The webhook calls `handle_drive_webhook(event)` which queues the relevant `sync_vector_store` task. This minimizes redundant refreshes while meeting the “refresh at least daily” rule if the webhook fails (cron remains as fallback).
